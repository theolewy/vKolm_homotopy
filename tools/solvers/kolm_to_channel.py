...
class TimeStepper(CartesianTimeStepper):
    ...
    def _set_vars_coords_param_names(self):
        ...
        self.variables = ['u', 'v', 'uy', 'vy', 'p', 'c11', 'c12', 'c22', 'c33', 'c11y', 'c12y', 'c22y', 'c33y']
        self.noise_on_variables = ['psi', 'c11', 'c12']  # use psi to denote 2D streamfunction
    ...
    def build_domain(self, **kwargs):
            ...
            self.y_basis = de.Chebyshev('y', self.Ny, interval=(-np.pi*self.n, np.pi*self.n), dealias=3 / 2)
    ...
    def equations(self):
        ...
        # impose periodic conditions with semi-homotopy to channel flow. 
    ...
    def add_tasks(self, save_freq='slow', suffix='', subdir='', save_all_fields=True, mode='append'):
        ...
        snapshots.add_task("integ( U ** 2 + V ** 2, 'x', 'y') / area / 2", layout='g', name='<KE_lam>')
    ...
    def plot_snaps(self, subdirectory='', suffix_end='', plot_dev=True):
        ...
        if on_local_device():
            ...
            fname=f"snaps2D_W_{self.W}_Re_{self.Re}_rho_{self.rho}_eps_{self.eps}_beta_{self.beta}_L_{self.L}_Lx_{self.Lx:.4g}_Nx_{self.Nx}_Ny_{self.Ny}_{suffix_end}".replace('.', ',')
    ...
    def plot_metrics(self, subdirectory='', suffix_end=''):
        ...
        fname = 'norm' if on_local_device() else f"norm_W_{self.W}_Re_{self.Re}_eps_{self.eps}_rho_{self.rho}_beta_{self.beta}_L_{self.L}_Lx_{self.Lx:.4g}_Nx_{self.Nx}_Ny_{self.Ny}_{suffix_end}".replace('.', ',')
